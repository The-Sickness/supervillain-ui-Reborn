...
local CoreGlobalName        = C_AddOns.GetAddOnMetadata(..., HeaderFromMeta);
local AddonVersion          = C_AddOns.GetAddOnMetadata(..., "Version");
...
local function HandleAddOns(PRIVATE_SV, CoreObject, THEME_KEY, HeaderFromMeta, SchemaFromMeta, ThemeFromMeta, LoadOnDemand)
    for i = 1, C_AddOns.GetNumAddOns() do
        local addonName, _, _, _, _, reason = C_AddOns.GetAddOnInfo(i)

        if(C_AddOns.IsAddOnLoadOnDemand(i)) then
            local header = C_AddOns.GetAddOnMetadata(i, HeaderFromMeta)
            local schema = C_AddOns.GetAddOnMetadata(i, SchemaFromMeta)
            local theme  = C_AddOns.GetAddOnMetadata(i, ThemeFromMeta)
...
local function CorePreInitialize()
    ...
    -- CALL THE HELPER
    HandleAddOns(PRIVATE_SV, CoreObject, THEME_KEY, HeaderFromMeta, SchemaFromMeta, ThemeFromMeta, LoadOnDemand)
...
function lib:NewCore(gfile, efile, pfile, mfile, sfile)
    ...
    CoreObject.Schema               = C_AddOns.GetAddOnMetadata(CoreName, SchemaFromMeta);
    CoreObject.TitleID              = C_AddOns.GetAddOnMetadata(CoreName, HeaderFromMeta);
